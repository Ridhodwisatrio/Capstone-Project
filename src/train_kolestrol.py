...
# Load Data
X_train = pd.read_csv('../data/X_train_smote_HighChol.csv')
y_train = pd.read_csv('../data/y_train_smote_HighChol.csv').values.flatten()
...
joblib.dump(scaler, "../model/scaler_HighChol.pkl")
...
def train_model(X_train, y_train, X_test, y_test, epochs=50):
    ...
    target_names = ['Non-HighChol (Sehat)', 'HighChol']
    print("\n=== PERFORMANCE REPORT PENYAKIT KOLESTROL ===")
...
trained_model.save("../model/HighChol_model.keras")