...
y_train = pd.read_csv('../data/y_train_smote_Diabetes_binary.csv').values.flatten()
...
joblib.dump(scaler, "../model/scaler_Diabetes_binary.pkl")
...
def train_model(X_train, y_train, X_test, y_test, epochs=50):
    ...
    y_pred = (y_pred_prob > 0.42).astype(int).flatten()

    target_names = ['Non-Diabetes (Sehat)', 'Diabetes']
    print("\n=== PERFORMANCE REPORT PENYAKIT DIABETES ===")
...
trained_model.save("../model/Diabetes_binary_model.keras")